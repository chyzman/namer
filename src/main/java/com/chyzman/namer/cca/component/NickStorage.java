package com.chyzman.namer.cca.component;
...
public class NickStorage implements Component, AutoSyncedComponent {
    ...
    public Text getNick(PlayerEntity player) {
        return this.nicks.get(player.getUuid());
    }

    public Map<UUID, Text> allNicks() {
        return this.nicks;
    }

    public boolean isUnique(Text nick) {
        return this.nicks.values().stream().noneMatch(thatNick -> raw(thatNick).equals(raw(nick)));
    }

    public static String raw(Text nick) {
        return nick.getString();
    }

    public static String formatForCommand(Text nick) {
        var returned = raw(nick);
        if(returned.contains(" ") || returned.contains("*")) returned = "\"" + returned + "\"";
        return returned + "*";
    }
}
